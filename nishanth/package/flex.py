...
class Flex(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = CustomLabel(self, text="FLEX", bg="#A294F9", fg="#1B1833", cursor="hand2", font=("Arial", 45))
        ...
        flex_size = tk.Label(self, text="FLEX SIZE :", bg="#A294F9", fg="#1B1833", cursor="hand2", font=("Arial", 35))
        flex_size.pack(pady=10, padx=10)
        flex_size.place(x=350, y=250)
        self.entry_flex_size = tk.Entry(self, width=20, font=('Arial', 35))
        self.entry_flex_size.pack(pady=10, padx=10)
        self.entry_flex_size.place(x=900, y=250)
    ...
    def insert_data(self):
        ...
        flex_size = self.entry_flex_size.get()
        customer_name = self.entry_customer_name.get()
        location = self.entry_location.get()
        amount = self.entry_amount.get()

        # Message box for validation
        if not date or not flex_size or not customer_name or not location  or not amount:
        ...
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS flex (date TEXT, size TEXT, customer_name TEXT, location TEXT, amount TEXT)")
        cursor.execute("INSERT INTO flex (date, size, customer_name,location, amount) VALUES (?, ?, ?, ?, ?)", 
                       (date, flex_size, customer_name, location , amount))
        conn.commit()
        conn.close()
        ...
        # Call clear_fields() to reset all fields
        self.clear_fields()

    def clear_fields(self):
        """This function clears all the input fields after data submission."""
        print("Clearing fields...")  # Debug line to confirm method is called
        self.entry_date.set_date("")  # Clear the calendar field
        self.entry_flex_size.delete(0, tk.END)  # Clear the flex size field
        self.entry_customer_name.delete(0, tk.END)  # Clear the customer name field
        self.entry_location.delete(0, tk.END)  # Clear the location field
        self.entry_amount.delete(0, tk.END)  # Clear the amount field
