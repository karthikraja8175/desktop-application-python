...
class Frame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = CustomLabel(self, text="FRAME", bg="#A294F9", fg="#1B1833", cursor="hand2", font=("Arial", 45))
        ...
        frame_size = tk.Label(self, text="FRAME SIZE :", bg="#A294F9", fg="#1B1833", cursor="hand2", font=("Arial", 35))
        frame_size.pack(pady=10, padx=10)
        frame_size.place(x=350, y=250)
        self.entry_frame_size = tk.Entry(self, width=20, font=('Arial', 35))
        self.entry_frame_size.pack(pady=10, padx=10)
        self.entry_frame_size.place(x=900, y=250)
    ...
    def insert_data(self):
        ...
        frame_size = self.entry_frame_size.get()
        customer_name = self.entry_customer_name.get()
        location = self.entry_location.get()
        amount = self.entry_amount.get()

        # Message box for validation
        if not date or not frame_size or not customer_name or not location  or not amount:
        ...
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS frame (date TEXT, size TEXT, customer_name TEXT, location TEXT, amount TEXT)")
        cursor.execute("INSERT INTO frame (date, size, customer_name,location, amount) VALUES (?, ?, ?, ?, ?)", 
                       (date, frame_size, customer_name, location , amount))
        conn.commit()
        conn.close()
        ...
        self.entry_frame_size.delete(0, tk.END)
